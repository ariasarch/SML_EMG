...
# -*- coding: utf-8 -*-
"""
Created on Sat May  6 18:32:39 2023
...
# Run model
def adaboost(best_params):
    model = AdaBoostClassifier(n_estimators = int(best_params['n_estimators']), learning_rate = best_params['learning_rate'], random_state=42)
...
# Run Main Function
def exec_adaboost(X_train, X_test, y_train, y_test):

    # Call bounds for optimization 
    pbounds = get_hyperparameter_space()
    
    # Evaluate model
    def evaluate_model(n_estimators, learning_rate):
