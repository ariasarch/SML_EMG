...
def knn_model(best_params):
    model = KNeighborsClassifier(n_neighbors = int(best_params['n_neighbors']), weights='distance', metric = 'cosine')
...
# Run Main Function
def exec_KNN_weighted(X_train, X_test, y_train, y_test):
    ...
    def evaluate_model(n_neighbors):
        ...
        # Run KNN model
        model = KNeighborsClassifier(n_neighbors = int(n_neighbors), weights='distance', metric = 'cosine')
