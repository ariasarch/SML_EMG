...
# Run Extra Tree  model
def extra_trees_classifier(best_params):
    model = ExtraTreesClassifier(n_estimators = int(best_params['n_estimators']), max_depth = int(best_params['max_depth']), 
...
def exec_extra_trees(X_train, X_test, y_train, y_test):
    ...
    def evaluate_model(n_estimators, max_depth, min_samples_split):
       ...
       # Run model
        model = ExtraTreesClassifier(n_estimators = int(n_estimators), max_depth = int(max_depth), min_samples_split = int(min_samples_split), random_state=42)
    ...
    model = extra_trees_classifier(best_params)
