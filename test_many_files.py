...
model_search_df = [pd.read_csv(file, index_col=0) for file in csv_files]

# After finishing all models testing, evaluate them
average_df = se.calculate_averages(model_search_df, base_path, filename='All_Model_Averages.csv', non_average_columns=['Model Name', 'Iteration', 'Time'])
best_model, best_eval = se.pick_best_model(average_df)
print(best_model, best_eval)